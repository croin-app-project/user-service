...
func (r *UserGormRepository) FindByCredential(username string, password string) (*domain.User, error) {
	var user domain.User
	if err := r.db.Where(&domain.User{Username: username, PasswordHash: password, IsActive: true}).First(&user).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (r *UserGormRepository) IsExistsByUsername(username string) (bool, error) {
	var user *string
	if err := r.db.Where(&domain.User{Username: username, IsActive: true}).Select("username").First(&user).Error; err != nil {
